export default class Utils {
        ...
        if ((process.env.NODE_ENV || 'development') == 'production' && (process.env.LOG_LEVEL || 'debug') == 'production') {
            if (values[0]) {
                ...
                if ((values instanceof Error
                    ...
                    || (tp == 'string' && (values[0] == 'FL' //FORCE LOG                        
    ...
    static firstValid(arr_valores, check_null = true) {
        try {
    ...
    static arrayToObject(array, key = null) {
        let result = {};
        if (array && key) {
            if (Utils.typeOf(key) == 'array') {
                ...
                for (let i in array) {
                    ...
                    for (let j = 1; j < key.length; j++) {
                        ...
                        if (j == key.length - 1) {
                            if (Utils.typeOf(currentPath[array[i][key[j]]]) !== 'array')
